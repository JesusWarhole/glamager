...
>
> Τελευταία ενημέρωση: **27/08/2026 (πρωί) — ΚΡΙΣΙΜΗ ΔΙΟΡΘΩΣΗ: το Hair Corner ΔΕΝ έχει μπει ακόμα σε πραγματική καθημερινή χρήση.** Μόνο test mode + δοκιμές μέχρι τώρα. Πραγματική έναρξη προγραμματισμένη **αρχές Σεπτεμβρίου 2026**. Αυτό αλλάζει ριζικά τη σειρά του πλάνου — βλ. παρακάτω.
>
...
2. **Tenant μοντέλο** — σχέδιο `users/{uid}/memberships/{tenantId}` + `/tenants/{tenantId}/...`. **ΕΠΕΙΓΟΝ ΤΩΡΑ.**
3. **Auth / memberships / roles** — πραγματικό invite flow αντί για χειροκίνητη δημιουργία λογαριασμών στο Firebase Console. **ΕΠΕΙΓΟΝ ΤΩΡΑ.**
4. **Security Rules rebuild** — πάνω στη νέα δομή, per-record όπου γίνεται. Το `pins` node κλειδώνει Master-only εδώ μέσα (μικρό, ήδη αναγνωρισμένο fix). **ΕΠΕΙΓΟΝ ΤΩΡΑ.**
5. **Per-record data model** — αντικατάσταση του whole-array `.set()` με `/visits/{id}` + `update()` ανά εγγραφή. Το πιο μεγάλο τεχνικό κομμάτι, αγγίζει πολλά σημεία στον κώδικα. **ΕΠΕΙΓΟΝ ΤΩΡΑ — γίνεται πολύ πιο εύκολο τώρα με άδεια δεδομένα παρά αργότερα με ζωντανά.**
