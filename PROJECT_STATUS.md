...
>
> Τελευταία ενημέρωση: **27/08/2026 (πρωί) — καμία αλλαγή κώδικα από 26/08 βράδυ. Ολόκληρη η χθεσινοβραδινή συζήτηση ήταν ΣΤΡΑΤΗΓΙΚΟΣ σχεδιασμός (με ChatGPT + Ανδρέα + Claude μαζί) — το πλάνο παρακάτω είναι το ΤΕΛΙΚΟ, κλειδωμένο αποτέλεσμα. Ξεκινάμε σήμερα την πραγματική υλοποίηση, Φάση 1.**
>
...
**Το τελικό, κλειδωμένο πλάνο φάσεων:**

1. **Backup/export** — πλήρες αντίγραφο υπαρχόντων δεδομένων πριν αγγίξουμε οτιδήποτε.
2. **Tenant μοντέλο** — σχέδιο `users/{uid}/memberships/{tenantId}` + `/tenants/{tenantId}/...`. Ακόμα σχέδιο, όχι live αλλαγή.
3. **Auth / memberships / roles** — πραγματικό invite flow αντί για χειροκίνητη δημιουργία λογαριασμών στο Firebase Console.
4. **Security Rules rebuild** — πάνω στη νέα δομή, per-record όπου γίνεται. Το `pins` node κλειδώνει Master-only εδώ μέσα (μικρό, ήδη αναγνωρισμένο fix).
5. **Per-record data model** — αντικατάσταση του whole-array `.set()` με `/visits/{id}` + `update()` ανά εγγραφή. Το πιο μεγάλο τεχνικό κομμάτι, αγγίζει πολλά σημεία στον κώδικα.
6. **Migration** — export → transform → validate (αριθμοί πριν = αριθμοί μετά, ΟΧΙ μάτι) → cutover. Το Hair Corner γίνεται `tenant_001`. **Χρειάζεται προγραμματισμένο βράδυ διακοπής** (π.χ. Κυριακή όταν κλειστό) — ρητά αναβλήθηκε στο τέλος της 26/08 να μη γίνει κουρασμένος.
7. **Offline / multi-device αξιοπιστία** — πιο ελαφρύ από το πλήρες IndexedDB engine που πρότεινε αρχικά το ChatGPT.
8. **Εμπορικό επίπεδο** — trial/subscription/Stripe/billing/onboarding νέου μαγαζιού. Μόνο όταν το θεμέλιο (1-7) δουλεύει σωστά.
