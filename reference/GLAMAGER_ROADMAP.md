...
- [x] 1a. Backup/export πριν migration — ΠΕΡΙΤΤΟ, καμία πραγματική εγγραφή ακόμα (το Hair Corner δεν έχει μπει σε πραγματική χρήση, μόνο test mode)
- [ ] 1b. Firebase Backups tab (Console → Realtime Database → Backups → Get started) — ΑΚΟΜΑ δεν έχει ενεργοποιηθεί, χρειάζεται πριν Σεπτέμβριο (βλ. περιστατικό 27/08 — `resetPreLaunch` άδειασε nodes, δεν υπήρχε δίχτυ)
...
Όχι ακόμα (P0 #1b, #3-#5 ανοιχτά) — αλλά πολύ πιο κοντά μετά το #2.
